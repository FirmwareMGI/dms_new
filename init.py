...
import rtb_compact as rtbcompact
import time
...
def relayType():
    ...
    for i in range(0, len(relay_type)):
        ...
        # RTBCOMPACT
        relay[6].append(rtbcompact.type_relay[i])
        typeRelay[6].append(rtbcompact.relayTeks[i])
        index[6].append(rtbcompact.indexMaster[i])
        mode[6].append(rtbcompact.mode[i])
        bd[6].append(rtbcompact.baudrate[i])
        bytesize[6].append(rtbcompact.byte_size[i])
        parity[6].append(rtbcompact.parity[i])
        stopbit[6].append(rtbcompact.stop_bit[i])
        portaddress[6].append(rtbcompact.port_address[i])
        print(typeRelay)
        # RTBCOMPACT
    ...
    for i in range(0, 2):
        for j in range(0, len(relayfinal[i])):
            ...
            # RTB COMPACT
            elif relayfinal[i][j] == 'RTBCOMPACT' and typeRelayFinal[i][j] == 'RTBCOMPACT':
                # print('RTBCOMPACT')
                for k in range(0, len(indexfinal1[i])):
                    if str(indexfinal1[i][k]) == indexfinal[i][j]:
                        flag = False
...
while True:
    ...
    #                     cekServer()
                ...
                # RTBCOMPACT
                elif relayfinal2[j][i] == 'RTBCOMPACT' and typeRelayFinal2[j][i] == 'RTBCOMPACT':
                    print('RTBCOMPACT')
                    rtbcompact.dataReg(idPort, counter[6])
                    counter[6] += 1
                # RTBCOMPACT
...
#                     micomp141.flag1 = False
    ...
    if int(dbdtList[0]) == 1:
        ...
        mycursor = mydb.cursor()
        sql = "UPDATE flag_config SET flag = '" + \
