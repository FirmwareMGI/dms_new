...
def dataReg( idPort, index) : 
    ...
    if connection:
        if dtRegister[ctRegister[idPort][index]] == 15616:
            ...
            #mon.readMeasurement(client[idPort][index],unit)
            mon.readMeasurementSingle(client[idPort][index],unit)
            flagModbus = 0
...
#             print(dtRegister[ctRegister[idPort][index]])
        ...
        if flagModbus == 0 :
            ...
            if baca.isError():
                ...
                read_milis_atas = client[idPort][index].read_holding_registers(address = 400, count = 1,unit= int(id_micom[idPort][index]))
...
def kirimServer(data, idPort, index) :
    ...
    if flagModbus == 7 and ctVariabel[idPort][index] <= 5 :
        ...
        if ctVariabel[idPort][index] > 5 :
            ...
            bentukFile(idPort,index,indexMaster[idPort][index])
...
#         statusRelay[idPort][index] = stringToList[2] 
        ...
        mycursor = mydb.cursor()
        sql = "SELECT kode_mesin FROM m_mesin"
        mycursor.execute(sql)
        dbtes = mycursor.fetchall()
        mydb.commit()
        kode_mesin = ''
        for i in dbtes:
            kode_mesin = i[0]
        print('kode_mesin: ', kode_mesin)
        
        for i in dbDR:
            dbCfg3[0] = i[5]
            dbCfg3[1] = i[7]
        bufwaktu= str(stringToList[3]).split(" ")
        dates =    bufwaktu[0].split("/")
        waktus = f"{dates[2]}/{dates[1]}/{dates[0]} {bufwaktu[1]}"
        

        if idPort == 0:
            d= {"machineCode":str(kode_mesin)
       ...
       # print(response)
        try:
            r = requests.post("http://"+str(dbCfg3[0])+"/mel/dms/fail/add", data=d)
            r.raise_for_status()
...
def bentukFile(idPort,index,portNumber):
    ...
    #print(dirName)       
    f = open(""+dirName+"/"+str(dirName)+".dat","w")
    f.write(tes1)
    f.close()
    ...
    f = open(""+dirName+"/"+str(dirName)+".cfg","w")
    f.write(tes1)
    f.close()
    tes1 = ''
    #exit()
    
    #with ZipFile('/home/pi/dms/DMSv1.2/zipDR/'+ dirName +'.zip', 'w') as zipObj: #perubahan 15-07-2024
    with ZipFile('/var/www/html/dms_setting/assets/api/file_dr/'+ dirName +'.zip', 'w') as zipObj: #perubahan 15-07-2024
    ...
    #dr_file = '/home/pi/dms/DMSv1.2/zipDR/'+ dirName +'.zip' #perubahan 15-07-2024
    dr_file = '/var/www/html/dms_setting/assets/api/file_dr/' + dirName +'.zip' #perubahan 15-07-2024
    print("index cuy",portNumber)
    hconn_send(0,dr_file,dirName,portNumber ,str(statusRelay[idPort][index]))
    print(int(indexFileDR[idPort][index]))
    if int(indexFileDR[idPort][index]) < 10:
        ...
        print(tes[0])
        #if os.path.exists('/home/pi/dms/DMSv1.2/zipDR'): #perubahan 15-07-2024
        if os.path.exists('/var/www/html/dms_setting/assets/api/file_dr/'): #perubahan 15-07-2024
          print("removed Files")  
        ...
        #files={'document':open('/home/pi/dms/DMSv1.2/zipDR/'+ dirName +'.zip','rb')} #perubahan 15-07-2024
        #files={'document':open('/var/www/html/dms_setting/assets/api/file_dr/'+ dirName +'.zip','rb')}
        #resp = requests.post('https://api.telegram.org/bot1607521967:AAErjzP0St1Y2_p44ZsWwJgIzXEjQJNsHTQ/sendDocument?chat_id=495884772',files = files)    
        #try :
        #    resp = requests.post('https://api.telegram.org/bot1607521967:AAErjzP0St1Y2_p44ZsWwJgIzXEjQJNsHTQ/sendDocument?chat_id=-4059643007&caption=Silahkan download File Disturbance Record diatas\n\nRelay ID : 1\nLokasi    : SURABAYA\nStatus    : '+str(statusRelay[idPort][index])+'\nTanggal : '+dateName+' WIB',files = files,) 
        #resp = requests.post('https://api.telegram.org/bot866502411:AAE7efXmuRG6KSOfu1hB275pEplEKMrAq5E/sendDocument?chat_id=712281930',files = files)
        #    resp = requests.post('https://api.telegram.org/bot866502411:AAErjzP0St1Y2_p44ZsWwJgIzXEjQJNsHTQ/sendDocument?chat_id=-4059643007&caption=Silahkan download File Disturbance Record diatas\n\nRelay ID : 1\nLokasi    : SURABAYA\nStatus    : '+str(statusRelay[idPort][index])+'\nTanggal : '+dateName+' WIB',files = files,)
        ...
        #EVENT P123
#        eventp123.fileName=dateName
#        eventp123.eventp123(0,0).pollEvent(idPort,int(id_micom[idPort][index]))
#        print(str(idPort)+"=====")
#        sql = "SELECT fileEvent.*,device_list.id_device as id_device_list FROM fileEvent inner JOIN device_list on fileEvent.port_device = device_list.port_number  WHERE fileEvent.port_device='0' and device_list.id_device=fileEvent.id_device"
#        mycursor.execute(sql)
#        dbEv = mycursor.fetchall()
#        mydb.commit()
#        #print(str(waktuFileDR[idPort][index]))
#        #print(buffstatusFileDR)
#        if(len(dbEv)<1):
#            waktu =dateName
#            evenName=dateName+'.eve'
#            sql = "INSERT into fileEvent (port_device,id_device,data,jumlah_data,waktu,lokasi,status) SELECT port_number,id_device,'"+evenName+"',1,'"+waktu+"','"+bufflokasiFileDR+"','"+buffstatusFileDR+"' FROM device_list WHERE port_address='"+str(idPort)+"' "
#            mycursor.execute(sql)
#            mydb.commit()
#        else:
#            newStatus=str(buffstatusFileDR)
#            newWaktu = dateName
#            newEvenName=dateName+'.eve'
#            portDevice=dbEv[0][0]
#            idDevice=dbEv[0][1]
#            evenName=dbEv[0][2]+','+newEvenName
#            status =str(dbEv[0][4])+','+newStatus
#            waktu =dbEv[0][5]+','+newWaktu
#            buffJmlEven=evenName.split(",")
#            jumlahLogEven = len(buffJmlEven)
#            sql="UPDATE fileEvent set jumlah_data = '"+str(jumlahLogEven)+"',data='"+evenName+"',waktu='"+waktu+"',status='"+status+"' where port_device = '"+portDevice+"' and id_device='"+idDevice+"'"
#            print(dbEv)
#            mycursor.execute(sql)
#            mydb.commit()
#
...
#        #perubahan terbaru
#        try :
#            files={'document':open('/home/pi/dms/DMSv1.2/dataEV/'+dateName+'.eve','rb')}
#            resp = requests.post('https://api.telegram.org/bot1607521967:AAErjzP0St1Y2_p44ZsWwJgIzXEjQJNsHTQ/sendDocument?chat_id=-4059643007&caption=Silahkan download File Event Record diatas',files = files,)
#        except:
#            print("request err")
#        ev_file = '/home/pi/dms/DMSv1.2/dataEV/'+ dateName +'.eve'
#        
#        hconn_send(1,ev_file,dateName,str(idPort) ,str(statusRelay[idPort][index]))
...
def hconn_send(tipe_url,dr_file,dirName,portNumber,status) :
    dbCfg = ''
    print("idPort: ",portNumber)
    print(dr_file)
    print("====")
    print(status)
    print("##########")
    mycursor = mydb.cursor()
    sql = "SELECT  network.* ,device_list.*from device_list cross join network where device_list.port_type = 0 and port_number = "+str(portNumber)
    print(sql)
    mycursor.execute(sql)
    dbDR = mycursor.fetchall()
    mydb.commit()
    buffName = dirName.split(" ")
    bufftanggal = buffName[0].split(".")
    tanggal=bufftanggal[2]+"-"+bufftanggal[0]+"-"+bufftanggal[1]
    buffwaktu =buffName[1].split(".")
    waktu =buffwaktu[0]+":"+buffwaktu[1]
    ...
    url=""
    if(tipe_url==0):
        url = 'http://'+str(dbCfg)+'/mel/dms/notif'
    else:
        url = 'http://'+str(dbCfg)+'/mel/dms/notif'
    file = {'myfile': open(dr_file,'rb')}
    dataReq={'status':status,'file_name':dirName,'machine_code':str(machineCode),'lokasi':lokasi,'type':type,'tanggal':tanggal,'waktu':waktu,'relay_id':relay_id,'port':portNumber}
    print(dataReq)
    status=str(dataReq).replace("'", "\"")
    try:
        r = requests.post(url,  data=dataReq,files=file,verify=False)
        if r.status_code != 200:
            print('sendErr: '+r.url)
            #perubahan 15-07-2024
            #query= f"insert into it_file_iec (domain_id,item_id,id_device,ip_address) values('{domainId}','{itemId}','{relayId}','{ip}')"
            sql = f"INSERT INTO fileDR_temp (port_device,id_device,status,flag,nama) values('{portNumber}','{relay_id}','{status}','{0}','{dirName}')"
            mycursor.execute(sql)
            mydb.commit()
            #perubahan 15-07-2024
        else :
            ...
            #perubahan 15-07-2024
            #query= f"insert into it_file_iec (domain_id,item_id,id_device,ip_address) values('{domainId}','{itemId}','{relayId}','{ip}')"
            sql = f"INSERT INTO fileDR_temp (port_device,id_device,status,flag,nama) values('{portNumber}','{relay_id}','{status}','{1}','{dirName}')"
            mycursor.execute(sql)
            mydb.commit()
            #perubahan 15-07-2024
