...
def readMeasurement(con, units):
    ...
    try:
        ...
        print(baca3.registers)
        param = ["IA", "IB", "IC", "IN"]
        #param2=["trip","alarm","healthy","t1","t2","t3","t4"]
...
# Tahap4
        ...
        for i in range(len(param)):
            idModbus = str(units)
            if(baca.registers[j+1]==1):
                dataOut = {
                    f"alias": f"{mesin_mesin}{str(units)}{param[i]}", "val": (baca.registers[j]+baca3.registers[0])/100, "dataType": "float"}
            else:
                dataOut = {
