...
def ClientFile61850_dir(con, iec_dir):
    ...
    if error_file.value != lib61850.IED_ERROR_OK:
        ...
        # print("directoryEntry: " + str(directoryEntry))
        test = lib61850.LinkedList_getData(rootDirectory)
        # print(test)
        # test = ctypes.cast(lib61850.LinkedList_getData(directoryEntry), lib61850.FileDirectoryEntry)
        # print("test: "+ str(test))
...
def CLient61850_monitoringRead(con, hostname, error, object_read, object_fc):
    if (error.value == lib61850.IED_ERROR_OK):
        print("object_read: %s" % object_read)
...
#     # Track extensions found for this file group
#     ExtensionFinal = []
#     for v in listFileNew:
#         for e in extension_file_dr:
...
#     # Process each file
#     for e in ExtensionFinal:
#         ClientFile61850_getFile(conn, FileFinal + e, directory, relayId)

#     # After files are downloaded, combine all ZIPs into one folder
#     combinedDir = f"/var/www/html/dms_setting/assets/api/file_dr/{relayId}/{FileFinal}_combined"
#     if not os.path.exists(combinedDir):
#         os.mkdir(combinedDir)

#     # Move all files (including both ZIPs) into the combined folder
#     for file_entry in listFileNew:
#         fileName = file_entry[0]
#         sourceDir = f"/var/www/html/dms_setting/assets/api/file_dr/{relayId}/{FileFinal}"

#         # Ensure the source directory exists
#         if os.path.exists(sourceDir):
#             sourcePath = os.path.join(sourceDir, fileName)
#             if os.path.exists(sourcePath):
#                 shutil.move(sourcePath, os.path.join(combinedDir, fileName))

#     # Optional cleanup: remove the original directory if empty
#     if os.path.exists(sourceDir) and not os.listdir(sourceDir):
#         os.rmdir(sourceDir)

#     print(f"âœ… All files combined into: {combinedDir}")
...
def restart_connection(device_index):
    """
    Attempts to restart the connection for a specific IEC61850 device.
    """
    print(f"Restarting connection for device {device_index} ({hostname[device_index]['hostname']})...")

    # Destroy the current connection if it exists
    if con[device_index]['con'] is not None:
        lib61850.IedConnection_destroy(con[device_index]['con'])
        con[device_index]['con'] = None

    # Create a new connection
    new_con = lib61850.IedConnection_create()
    lib61850.IedConnection_connect(new_con, ctypes.byref(error[device_index]['error']), hostname[device_index]['hostname'], int(tcpport[device_index]['port']))

    # Check if the connection was successful
    if lib61850.IedConnection_getState(new_con) == lib61850.IED_STATE_CONNECTED:
        con[device_index]['con'] = new_con
        print(f"Successfully reconnected to {hostname[device_index]['hostname']}.")
        return True
    else:
        print(f"Failed to reconnect to {hostname[device_index]['hostname']}. Skipping device.")
        return False
...
def process_disturbance_files(device_index):
    ...
    if currentFileList is None:
        print(f"âŒ File list retrieval failed for client {device_index}")
        #con[device_index]['con'] = None
        return
    ...
    # print("ðŸ“ Existing data:", existing_bases)
    ...
    if not new_bases:
        print(f"âœ… No new files to process for device {device_id}")
        return

    print(f"ðŸ› ï¸ New files detected for device {device_id}: {new_bases}")
    ...
    for base_name in new_bases:
        print(f"âœ… Processing new file group: {base_name}")
        ...
        if isSuccess:
            ...
            print(f"âŒ Failed to process {base_name}, marking as failed.")
            break
    ...
    try:
        ...
        if device_entry is not None and isinstance(device_entry, str) and device_entry.strip():
            print(f"ðŸ”§ Updating DR data for device {device_id}")
            ...
            print(f"ðŸ†• Inserting DR data for device {device_id}")
            db.insertDb.data_iec_filedr_insert(0, device_id, updated_base_list_str)
            

        print(f"âœ… All new files processed and DB updated for device {device_id}")

        # Update the local cache
        iecFiles[device_index]['files'] = updated_base_list_str

    except Exception as e:
        print(f"âŒ Error updating DB for device {device_id}: {e}")
...
def enforce_file_limit(device_id, current_base_name):
    ...
    ðŸš¨ **DO NOT** update the DB when files are deleted to prevent duplicates!
    """
    device_folder = f"/var/www/html/dms_setting/assets/api/file_dr/{device_id}/"

    # Check if the device folder exists
    if not os.path.exists(device_folder):
        print(f"âš ï¸ Device folder not found: {device_folder}")
        return
    ...
    if max_files is None:
        print(f"âš ï¸ No file limit set for device {device_id}")
        return
    ...
    if len(files_in_folder) > max_files:
        ...
        print(f"ðŸ—‘ï¸ Deleting {len(files_to_delete)} old files to maintain the limit:")
        for old_file in files_to_delete:
            try:
                old_file_path = os.path.join(device_folder, old_file)
                if os.path.exists(old_file_path):
                    os.remove(old_file_path)
                    print(f"ðŸ—‘ï¸ Deleted zip file: {old_file_path}")
                ...
                print(f"âŒ Error deleting file or folder {old_file}: {e}")
    else:
        print("âœ… No files to delete.")


def read_monitoring_data(device_index):
    """Read monitoring data from IEC61850 and prepare for MQTT publishing."""
    mqtt_data = []

    if 'obj' in object_read[device_index]:
        for j in range(len(object_read[device_index]['obj'])):
            value, types, isSuccess = CLient61850_monitoringRead(
                con[device_index]['con'], hostname[device_index]['hostname'], error[device_index]['error'], 
                object_read[device_index]['obj'][j], object_FC[device_index]['obj'][j])
            
            if not isSuccess:
                #con[device_index]['con'] = None
                print(f"Failed to read monitoring data for device {device_index}. Restarting connection...")
                return None  # Skip the rest of the objects
            
            if types is None or types == 0:
                continue  # Skip None values
            
            data_entry = {
                "alias": alias_monit[device_index]['alias'][j],
                "val": value,
                "dataType": types
            }
            topic = f"DMS/{mesin_mesin}/IEC61850/{object_read[device_index]['id']}/{object_read[device_index]['itemID'][j]}/"
            dataJson = str(data_entry).replace("'", "\"")
            mqtt_data.append((topic, dataJson))
...
try:
    while True:
        ...
        for i in range(0, max_client):
            ...
            #if not isOpen(hostname[i]['hostname'], tcpport[i]['port']):
            if lib61850.IedConnection_getState(con[i]['con']) == lib61850.IED_STATE_CLOSED:
                lib61850.IedConnection_destroy(con[i]['con'])
                print(f"Port closed for {hostname[i]['hostname']}:{tcpport[i]['port']}. Skipping this device.")
