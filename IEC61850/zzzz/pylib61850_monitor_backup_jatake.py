    ...
    if newfileList or newfileList != None or prevFileList or prevFileList != None:
        ...
        for download in newFileDownload:
            for ext in extensionList:
                ...
                # print(download+ext)
                ClientFile61850_getFile(conn, download+ext, directory)
        return newFileDownload, extensionList
    else:
        print("FILE LIST NONE")
        return None, None
...
def initializeFileDR():
    ...
    for i in range(max_client):
        ...
        if len(buff_dbFileDR) == 0:
            ret_dfr_list = dfr_checkingFiles(con[i]['con'], iecfolder[i]['id'])
        ...
        if time.time() - start_time_3s > 3:
            for i in range(0, max_client):
                if disturbanceType[i]['type'] == '1':
                    ...
                    if dfr_checking[1] != dfr_existing:
                        print("NEW FILE")
                        print("check ", dfr_checking[1])
                        print("existing ", dfr_existing)
                        extension = db_fileList[i][0]['extensionDR']
                        # dirName, extFile = newFileMechanismNew(
                        #     con[i]['con'], dfr_existing, dfr_checking[1], iecfolder[i]['id'], extension)
                        # print(dfr_checking[0])
                        #print(iecfolder[i]['id']+dfr_checking[0])
                        dirName, extFile = newFileMechanismNew(
                            con[i]['con'], dfr_existing, dfr_checking[1], iecfolder[i]['id']+dfr_checking[0], extension)

                        # print(dirName)
                        # print(extFile)

                        if dirName != None or extFile != None:
                            print("dirName is available")
                            if len(dirName) > 1:
                                for v in dirName:
                                    print(v)
                                    dr_fileZip = buildZip(v)
                                    try:
                                        DFRFailADD(
                        ...
                        # db.writeDb.updfileDRNEW(
                        #     0, con[i]['id'], updatedDFRList)
                        # db_fileList[i] = db.readDb.filedr(0, con[i]['id'])
                    #     listfileArr[i] = ClientFile61850_dir(
                    #         con[i]['con'], iecfolder[i]['id'])
...
#             for i in range(0, max_client):
#                 # update file latest
#                 print("HOSTNAME: ", hostname[i]['hostname'])
#                 if disturbanceType[i]['type'] == '1':
#                     if flags == 0:
#                         listfileArrPrev = listfileArr
#                         flags = 1
...
#         if time.time() - start_time_60s > 59:
#             for i in range(0, max_client):
#                 print("HOSTNAME: ", hostname[i]['hostname'])
...
#                         # print("counting object %d" % (j))
#                         # try:
#                         if object_type[i]['object_type'][j] == 'float':
#                             value, types = CLient61850_monitoringRead(
#                                 con[i]['con'], hostname[i]['hostname'], error[i]['error'], object_read[i]['obj'][j], object_FC[i]['obj'][j])
#                             dataJson = {
#                                 "alias": alias_monit[i]['alias'][j], "val": value, "dataType": types}
...
#         lib61850.Thread_sleep(3000)
#         for i in range(0, max_client):
#             if isOpen(hostname[i]['hostname'], 102):
#                 if flagCon == 0:
#                     print("if DESTROY PREVIOUS")
#                     lib61850.IedConnection_abort(
#                         con[i]['con'], error[i]['error'])
#                     # CLient61850_destroyConn(con[i]['con'], error[i]['error'])
#                     Client61850_connection(
#                         con[i]['con'], error[i]['error'], hostname[i]['hostname'], int(tcpport[i]['port']))
#                     flagCon = 1
#                 # update file latest
#                 if disturbanceType[i]['type'] == '1':
#                     if flags == 0:
...
#   flagCon = 0
#   if isOpen(hostname[i]['hostname'], 102):
#       lib61850.IedConnection_abort(con[i]['con'], error[i]['error'])
#       print("else DESTROY PREVIOUS")
#       Client61850_connection(
#           con[i]['con'], error[i]['error'], hostname[i]['hostname'], int(tcpport[i]['port']))
#       flagCon = 1
#         #time.sleep(0.5)
