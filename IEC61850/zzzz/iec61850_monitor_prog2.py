...
logging.basicConfig(format='%(asctime)s %(name)-10s %(levelname)-6s %(message)s',
                    level=logging.INFO)
...
def db_getDB():
    ...
    for db_val in buff_db_device:
        ...
        disturbance[db_val['id_device']]['type'] = db_val['iec_file']
        disturbance[db_val['id_device']]['path'] = db_val['disturbance_type']
...
def iec61850_readFile_directory(id_device: int, directory: str) -> tuple[int | float | bool, str]:
    ...
    if err == 0:
        ...
        if error.value == lib61850.IED_ERROR_OK:
            ...
            logger.error("error get list file directory")
            err = error.value
            # return None, None
            return None
...
def iec61850_downloadFile(id_device: int, filePath: str) -> tuple[int | float | bool, str]:
    ...
    directorySavedFile = '/home/pi/dms/DMSv1.2/IEC61850/DR_FILES/'
...
def send_DFRFailADD(id_device, rack, DFRPath, types):
    ...
    d = {"machineCode": systems["kode_mesin"],
         "relayId": str(id_device),
         "lokasi": systems["nama_gi"]+" "+database[id_device]['rack'],
...
def send_DFRUploadFile(id_device, rack, DFRPath, types):
    ...
    d = {"status": "DFR Available",
         "file_name": DFRName,
         "machineCode": systems["kode_mesin"],
         "lokasi": systems["nama_gi"]+" "+database[id_device]['rack'],
         "namaFile": DFRName,
         "waktu": stringTime,
         "flag": 0,
    ...
    if time.time() - time_3s > 3:
        for ied in connections:
            ...
            if err == 0:
                ...
                # print(bufferFiles)
                if bufferFiles != None:
                    print("bufferFiles Not Empty")
                    bufferFiles = set(bufferFiles)
                    if len(disturbance[ied]['files']) != 0:
                        prevBufferFiles = set(disturbance[ied]['files'])
                    else:
                        prevBufferFiles = set()
                    # print(prevBufferFiles)
                    ...
                    if newFiles:
                        ...
                        for itemFile in newFiles:
                            ...
                            if err == 0:
                                print("success download all")
                                folderFilePath = pathlib.Path(itemFile).stem
                                print(folderFilePath)
                                print(pack_buildZip(folderFilePath))
                                disturbance[ied]['files'].append(itemFile)
                                # print(disturbance[ied]['files'])
                                buff_updDB_listFIle = "|".join(
                                    disturbance[ied]['files'])
                                buff_updDB_Ext = "|".join(
                                    disturbance[ied]['ext'])
                                print(buff_updDB_listFIle,
                                      buff_updDB_Ext)
#                                db.writeDb.update_data_iec_filedr(
#                                    0, ied, buff_updDB_listFIle, buff_updDB_Ext, 0)
...
db_getDB()
db_itemConfig()
# buff_db_DR = db.readDb.data_iec_filedr(0, 'ALL')

for v in connections:
    buff_db_DR = db.readDb.data_iec_filedr(x=0, id_device=v)

    if  buff_db_DR == None or len(buff_db_DR) == 0:

        #buff_query = f"INSERT INTO data_iec_fileDR(id_device, jumlahDR, status, listDR, pathDR,fileDR,extensionDR,timeDR) VALUES('"+str(
        #    v)+"', '""','""', '""', '""', '""', '""', '""') "
        ...
        readQuery = db.readDb.data_iec_filedr(0, v)
        print(readQuery)
        disturbance[v] = {}
        disturbance[v]['type'] = {}
        disturbance[v]['files'] = []
