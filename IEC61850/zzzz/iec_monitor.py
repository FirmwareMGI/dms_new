...
if __name__ == '__main__':
   ...
   # print(dataDevice[0]["ip_address"])
    ...
    while True:
        ...
        ipserver=dataMeasurement[flagDataMeasurement]['ipserver']
        ...
        kode_wilayah="0001"
        if(dataMeasurement[flagDataMeasurement]['alias']=="TRIP"):
            print('ok')
            print(dataOut)
            #print
            if(dataOut==True and flagNotif==0):
                #kirim notif
                dateTime= datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                print("krim notif")
                d= {"machineCode":str('0001')
                    ,"relayId": str(2)
                    ,"lokasi": "GI WARU"
                    ,"status": "TRIP"
                    ,"namaFile": "RELAY Disturbance.000"
                    ,"waktu": dateTime
                    ,"flag": 0
                    ,"type": "SEL 849"
                    ,"portType": 1
                    ,"portNumber": "192.168.2.100"
                    }
                print(d)
                r = requests.post(" http://"+ipserver+"/dms/fail/add", data=d)
            if(dataOut==False):
                flagNotif=0

        topic = f"DMS/{kode_wilayah}/IEC61850/{dataMeasurement[flagDataMeasurement]['id_device']}/{dataMeasurement[flagDataMeasurement]['item_id']}/"
        try:
